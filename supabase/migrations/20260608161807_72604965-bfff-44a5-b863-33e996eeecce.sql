...
DECLARE
  ...
  caller_role TEXT;
BEGIN
  -- 🔒 Bloqueia chamadas de usuários autenticados comuns.
  -- Permitido apenas para: admin (via has_role) ou service_role (edge functions / pg_cron).
  caller_role := current_setting('request.jwt.claim.role', true);
  IF caller_role IS DISTINCT FROM 'service_role'
     AND NOT public.has_role(auth.uid(), 'admin'::app_role) THEN
    RAISE EXCEPTION 'Forbidden: upsert_partner_radar_memory requires admin or service_role';
  END IF;
