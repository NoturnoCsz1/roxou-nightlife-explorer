...
const NoticiasList = () => {
  ...
  const remove = async (id: string) => {
    ...
    const { error } = await supabase.from("expo_news").delete().eq("id", id);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-xl font-black font-display">Notícias / Expo</h1>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-border p-10 text-center text-sm text-muted-foreground">
          Nenhuma notícia ainda. Crie a primeira para alimentar o hot site.
        </div>
      ) : (
          ...
          {rows.map((r) => (
            <div key={r.id} className="flex items-center justify-between gap-3 rounded-xl border border-border/60 bg-card/60 p-3">
              ...
              <div className="flex items-center gap-1">
                {r.status === "published" && (
                  <a href={`/expo2026/noticia/${r.slug}`} target="_blank" rel="noreferrer" className="p-2 rounded-lg hover:bg-secondary text-muted-foreground hover:text-foreground" title="Ver">
