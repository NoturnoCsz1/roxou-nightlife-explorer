...
export const FULL_NAVIGATION: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: "/bio/compartilhar", label: "Compartilhar", icon: Link2, modes: MANAGER_LIKE, match: (p) => p.startsWith("/bio/compartilhar") },
      { to: "/analytics", label: "Analytics", icon: BarChart3, modes: MANAGER_LIKE, match: (p) => p.startsWith("/analytics") || p.startsWith("/bio/analytics") },
    ],
  },
  {
    id: "negocio",
    ...
    items: [
      ...
      { to: "/crm", label: "CRM", icon: Users2, modes: MANAGER_LIKE, match: (p) => p === "/crm" || p.startsWith("/crm/") },
      { to: "/relatorios", label: "Relatórios", icon: LineChart, modes: MANAGER_LIKE },
      { to: "/configuracoes#financeiro", label: "Financeiro", icon: Wallet, modes: MANAGER_LIKE, comingSoon: true },
    ],
  },
  {
    id: "configuracoes",
    ...
    items: [
      ...
      { to: "/configuracoes#integracoes", label: "Integrações", icon: Plug, modes: MANAGER_LIKE },
      { to: "/configuracoes", label: "Sistema", icon: Settings, modes: MANAGER_LIKE, match: (p) => p === "/configuracoes" || p.startsWith("/configuracoes/") },
      { to: "/configuracoes/avancado", label: "Conta", icon: Cog, modes: MANAGER_LIKE },
    ],
  },
];
