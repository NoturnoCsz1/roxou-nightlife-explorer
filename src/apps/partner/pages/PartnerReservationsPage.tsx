/**
 * PartnerReservationsPage — Fase 9H
 * Lista, cria e gerencia reservas do parceiro selecionado.
 */
...
import type { ReservationFiltersValue } from "../components/ReservationFilters";
...
const PartnerReservationsPage = () => {
  ...
  const [filters, setFilters] = useState<ReservationFiltersValue>({
    status: "all",
    search: "",
  });
  ...
  const handleQuickAdd = async () => {
    ...
    const people = Number(window.prompt("Quantas pessoas?", "2") || "2");
    try {
  ...
  return (
    <main className="min-h-screen space-y-6 p-4 sm:p-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex gap-2">
          {canCreate && (
            <Button onClick={handleQuickAdd}>Nova reserva</Button>
          )}
